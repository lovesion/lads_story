...
- “40 篇、逐篇独立 brief、2,000–4,000 字符、逐篇审校”是本次发布的完成标准，不是允许跳过或中止发布的理由。
- 不得以工作量大、创作耗时长、需要审校或担心质量为由，在尚未尝试完成内容管线时自行停止。应继续完成逐篇创作与审校；若确有外部故障或验证失败，记录具体错误并在可恢复后继续。
- 不得用脚本、固定句库、批量正文模板或降低字数来规避这些完成标准。若当前运行时长受限，应保留已完成稿件与 brief，并在下一次运行从未完成稿件继续，而非将当日任务标为已跳过。
